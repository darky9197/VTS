const table_data = require('./local_database/tabledata.json')
...
function generator(arr: DayData[]) {
    let temp = [...arr], result: DayData[] = [], last = 0;
    while (temp.length > 1) {
...
function SCIF(data: DayData[], settings) {
    let temp = [...data];
    const tabledata = {
...
function SCIF2(subs_data: DayData[], settings, day: string) {
    let period_count = 7, randval = 0, it = 0, temp:any = [[], []];
    const periods:{
        data:DayData[]
        class:string[]
        set:number[]
...
let arr: DayData[] = [], count_checker = 0;
